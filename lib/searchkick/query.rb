module Searchkick
  class Query
    ...
    def execute_search
      @respond || Searchkick.client.search(params)
    end
        ...
        if options[:query]
          ...
          if options[:autocomplete]
            ...
            if misspellings != false
              ...
              default_max_expansions = @misspellings_below ? 20 : 100
          ...
          if options[:relevance] && options[:relevance][:on] && conversions_field && options[:conversions] != false
            ...
            script_score =
              if below12?
                ...
                {field_value_factor: {field: "#{conversions_field}.count", factor: config[:factor], modifier: config[:modifier]}}
              end
